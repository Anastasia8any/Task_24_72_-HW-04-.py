from api import PetFriends # aggiungiamo api.py
from settings import valid_email, valid_password #aggiungiamo settings con le nostre credenziali
import os
import pytest
...
def test_add_new_pet_with_valid_data(name='Sasha', animal_type='Dog',
                                     age='4', pet_photo='images/husky.jpg'):
...
def test_add_new_pet_with_invalid_auth_key():
    ...
    status, result = pf.add_new_pet(invalid_auth_key, "Тест", "Собака", "5", "images/husky.jpg")
    assert status == 403
...
def test_add_pet_with_empty_name():
    """
        ...
        """
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, "", "Кот", "2", "images/leo.jpg")
...
def test_add_new_pet_with_empty_data():
    ...
    """
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, "", "", "", "images/leo.jpg")
...
# BUG: API некорректно принимает запрос на создание питомца без обязательного поля 'pet_photo'.
...
def test_add_pet_with_non_numeric_age():
    ...
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, "Буквы", "Кот", "пять", "images/leo.jpg")
...
def test_get_pets_with_invalid_auth_key():
    ...
    invalid_auth_key = {"key": "invalid_auth_key_12345"}  # Используем только ASCII символы
